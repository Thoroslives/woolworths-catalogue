...
describe("the multibuy printed on a product card", () => {
  ...
  it("keeps the deal when its unit price is absent", () => {
    const page = structuredClone(fixture("category-single-meals-multibuy")) as {
      data: { productsByCategory: { productsFeed: RawProductCard[] } };
    };
    const card = page.data.productsByCategory.productsFeed.find(
      (product) => String(product.productId).endsWith("388386")
    )!;
    delete card.multiBuyPriceInfo?.unitPrice;
  ...
  it("reads a promotion and an absent multibuy independently", () => {
  ...
  it("maps blank multibuy strings to null", () => {
    const row = mapProductCard(
