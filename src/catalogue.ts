...
import { readMultiBuy, readPromotion, type RawProductCard } from "./productMapper.js";
...
export function mapCategoryPage(
  ...
  for (const card of page?.productsFeed ?? []) {
    ...
    entries.push({
      ...
      // The same readers the product read uses, so a special or a deal means
      // the same thing whichever call found it.
      ...readPromotion(card),
      ...readMultiBuy(card),
